...
root_dir = 'mscnn-8s-768-trainval-pretrained/';    //模型文件的路径
binary_file = [root_dir 'mscnn_kitti_trainval_2nd_iter_35000.caffemodel'];//权重的路径
assert(exist(binary_file, 'file') ~= 0);　//确定网络权重的存在
definition_file = [root_dir 'mscnn_deploy.prototxt'];
assert(exist(definition_file, 'file') ~= 0);//确定网络部署文件的存在
...
net = caffe.Net(definition_file, binary_file, 'test');//初始化网络，为test模式

% set KITTI dataset directory 设置数据集的路径
...
% imgW = 1280; imgH = 384;
% imgW = 1920; imgH = 576;
imgW = 2560; imgH = 768;　//输入图像的尺寸

mu = ones(1,1,3); mu(:,:,1:3) = [104 117 123];　//均值文件
